...
class Deconv3DBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=3):
        super().__init__()
        self.block = nn.Sequential(
            nn.ConvTranspose3d(in_planes, in_planes, kernel_size=2, stride=2),
            nn.Conv3d(in_planes, out_planes, kernel_size=kernel_size, 
                               padding=((kernel_size - 1) // 2)),
            nn.BatchNorm3d(out_planes),
            nn.ReLU(True)
...
class Transformer(nn.Module):
    def __init__(self, input_dim, embed_dim, cube_size, patch_size, num_heads, num_layers, dropout, extract_layers, mlp_hidden):
        super().__init__()
        ...
        for _ in range(num_layers):
            layer = TransformerBlock(embed_dim, num_heads, dropout, cube_size, patch_size, mlp_hidden)
...
class UNETR(nn.Module):
    def __init__(self, img_shape=(128, 128, 128), 
                 input_dim=4, 
                 output_dim=3, 
                 embed_dim=768, 
                 patch_size=16, 
        ...
        self.decoder0_header = \
            nn.Sequential(
                Conv3DBlock(128, 64),
                Conv3DBlock(64, 64),
                nn.Conv3d(64, output_dim, kernel_size=1)
            )
